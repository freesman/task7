define(
  ['jquery', 'underscore', 'backbone', 'epoxy', 'models/Person', 'inputmask']
  ($, _, Backbone, Epoxy, Person) ->
    PersonView = Backbone.Epoxy.View.extend(
      ...
      clearing: ->
        ...
        this.helpClear(this.$('.date'))
      helpClear: (e) ->
        e.val('')
        e.css('border-color', '#A9A9A9')
      localSave: ->
      ...
      aliases: -> # mask templates for each attribute
        ...
        Inputmask.extendAliases(
          'myPhone':
            ...
            onincomplete: (e) ->
              if $(e.currentTarget).val() then $(e.currentTarget).css('border-color', 'red')
              else $(e.currentTarget).css('border-color', '#A9A9A9')
            oncomplete: (e) -> $(e.currentTarget).css('border-color', 'green')
        )
        Inputmask.extendAliases(
          'myEmail':
